...
func ObjectDownload(ctx *fasthttp.RequestCtx) {
	bucket := middleware.GetBucketFromRequest(ctx);
	if bucket == nil {
		return;
	}
	...
	switch condition {
		...
		// Verify the signature on the key before allowing access.
		case AllowSigned:
			log.Println("implement allowsigned pls i will bug you in console until you do");
			fallthrough;
		...
		case AllowPublic:
			...
			ctx.Response.Header.SetBytesV("ETag", object.ETag);
			...
			if etag := ctx.Request.Header.Peek(fasthttp.HeaderIfNoneMatch); len(etag) != 0 && bytes.Equal(etag, object.ETag) {
			...
			var readLength uint64 = object.Size;
			...
			if rangeHeader := ctx.Request.Header.Peek(fasthttp.HeaderRange); len(rangeHeader) != 0 {
				...
				if etag := ctx.Request.Header.Peek(fasthttp.HeaderIfRange); len(etag) == 0 || bytes.Equal(etag, object.ETag) {
					parsedRange, err := handlers.Misc.ParseRangeHeader(rangeHeader, object.Size);
					if err == nil {
			...
			file, err := os.Open(bucket.GetObjectPath(object.ObjectUid));
			if err != nil {
				log.Println(err);
				ctx.SetStatusCode(500);
				return;
